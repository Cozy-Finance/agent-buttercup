...
use bindings::cost_model_dynamic_level_factory;
...
pub fn run_growth_rate_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while cost_factor_delta <= growth_rate_config.max_rate_cost_factor_delta {
        ...
        let optimal_zone_rate = (cost_factor_delta * wad())
            / (growth_rate_config.width * growth_rate_config.rate_time_delta_in_seconds);
        dynamic_level_runner_mut.cost_models = vec![(
            "DynamicLevel".into(),
            CozyCostModelType::DynamicLevel(cost_model_dynamic_level_factory::DeployModelCall {
                u_low: growth_rate_config.u_opt - growth_rate_config.width,
                u_high: growth_rate_config.u_opt + growth_rate_config.width,
                cost_factor_at_full_utilization: growth_rate_config.cost_factor_at_full_utilization,
                cost_factor_at_zero_utilization: growth_rate_config.cost_factor_at_zero_utilization,
                cost_factor_in_optimal_zone: growth_rate_config.cost_factor_at_zero_utilization,
                optimal_zone_rate,
            }),
        )];
