...
pub struct Chronos<AgentState: AgentStatus> {
    /// Name of the agent.
    pub name: String,
...
impl<AgentState: AgentStatus> Identifiable for Chronos<AgentState> {
    fn name(&self) -> String {
        self.name.clone()
    }
}

impl Agent for Chronos<IsActive> {
