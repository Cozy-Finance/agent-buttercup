...
pub struct CozyPassiveSupplier<AgentState: AgentStatus> {
    /// Name of the agent.
    pub name: String,
...
impl<AgentState: AgentStatus> Identifiable for CozyPassiveSupplier<AgentState> {
    fn name(&self) -> String {
        self.name.clone()
    }
}

impl Agent for CozyPassiveSupplier<IsActive> {
...
impl CozyPassiveSupplier<NotActive> {
...
impl CozyPassiveSupplier<IsActive> {}
